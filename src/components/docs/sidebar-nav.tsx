    ...
    <div className="w-full">
      {items.map((item) => (
        <div key={item.title} className={cn("pb-8")}>
          <h4 className="mb-1 rounded-md py-1 text-base font-medium md:px-2 md:text-sm">{item.title}</h4>
          {item.items ? <DocsSidebarNavItems setOpen={setOpen} items={item.items} pathname={pathname} /> : null}
        </div>
      ))}
    </div>
  ) : null;
