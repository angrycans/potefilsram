...
const components = {
  ...
  pre: ({ className, __rawString__, ...props }: React.HTMLAttributes<HTMLPreElement> & { __rawString__?: string }) => (
    <div className="group relative w-full overflow-hidden">
      <pre
        className={cn("max-h-[650px] overflow-x-auto rounded-lg border bg-zinc-900 py-4 dark:bg-zinc-900", className)}
        {...props}
      />
      {__rawString__ && (
        <CopyButton
  ...
  const MDXImage = (props: any) => {
    ...
    return (
      <div className="mt-5 w-full overflow-hidden rounded-lg border">
        <BlurImage {...props} blurDataURL={blurDataURL} className="size-full object-cover object-center" />
      </div>
    );
  };
